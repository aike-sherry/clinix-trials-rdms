...
function PatientTable({ patients, projects }: { patients: any[]; projects: any[] }) {
  ...
  const [pageSize, setPageSize] = usePageSize('crf_pagesize_entry_patients')
  ...
  // 访视完成率：已完成访视 ÷ 应做访视（退出/失访患者按最后有数据的访视计应做数）
      ...
      <CardContent className="p-0">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
          <h3 className="text-sm font-medium text-slate-700 flex items-center gap-2">
            ...
            患者清单
            <span className="text-xs font-normal text-slate-400">共 {patients.length} 例</span>
          </h3>
        </div>
...
function SingleProjectView({ project, allPatients, allProjects }: { project: any; allPatients: any[]; allProjects: any[] }) {
  ...
  const monthlyData = useMemo(() => generateMonthlyPatientData(projectPatients), [projectPatients])
...
export default function EntryPatients() {
  ...
  const searchedPatients = useMemo(() => {
    ...
    return patients.filter((p) =>
      ...
      (p.nameInitials || '').toLowerCase().includes(search.toLowerCase()) ||
      (p.randomizationId || '').includes(search)
    )
  }, [patients, search])
  ...
  // 全部研究 / 单个项目
  if (selectedProjectNo === 'all' || !project) {
    return <AllProjectsView projects={projects} patients={searchedPatients} />
  }
  return <SingleProjectView project={project} allPatients={searchedPatients} allProjects={projects} />
}
